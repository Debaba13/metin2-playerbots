...
function Show-PanelChoiceDialog {
    ...
    $dialog = [Windows.Forms.Form]::new()
    $dialog.Text = 'Który panel otworzyć?'
    ...
    $info = [Windows.Forms.Label]::new()
    $info.Text = 'Oba panele pokazują ten sam świat i działają jednocześnie.'
    $info.Location = [Drawing.Point]::new(16, 14)
    $info.Size = [Drawing.Size]::new(480, 22)
    $dialog.Controls.Add($info)

    $classicButton = [Windows.Forms.Button]::new()
    $classicButton.Text = "Oryginalny panel`r`nmapa i sterowanie  ·  port $($Addresses.ClassicPort)"
    ...
    $sebanButton.Text = "Zaawansowany panel seban latino`r`nprofile, rankingi, gospodarka, obciążenie  ·  port $($Addresses.SebanPort)"
